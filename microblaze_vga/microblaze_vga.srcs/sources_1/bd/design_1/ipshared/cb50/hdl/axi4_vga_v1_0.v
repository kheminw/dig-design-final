...
`timescale 1 ns / 1 ps
	...
	(
		...
		parameter integer C_VIDEO_S00_AXIS_TDATA_WIDTH	= 12
	)
	(
...
//	);
    ...
    reg [11:0] rgb_reg;
    wire video_on;
    vga_test vga_inst(video_s00_axis_aclk, video_s00_axis_aresetn, rgb_reg, hsync, vsync, video_on, rgb_out);
    always @(video_s00_axis_aclk)
    begin
        if(!video_s00_axis_aresetn) rgb_reg <= 0;
        else if(video_s00_axis_tvalid) rgb_reg <= video_s00_axis_tdata[11:0];
    end 
    assign video_s00_axis_tready = video_on;   
	// User logic ends
